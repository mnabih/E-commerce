...
if (!function_exists('getAllParents')) {
	function getAllParents($did){
		$data = [];
		$Department = App\Model\Department::find($did);
		if($Department->parent != null and $Department->parent > 0 ){			
			while($Department->parent != null){
				$Department = App\Model\Department::find($Department->parent);
				array_push($data, $Department->id);
			}
			return $data;
		}else{
			return $data;
		...
		// if($Department->parent != null and $Department->parent > 0 ){
		// 	return getAllParents($Department->parent) . "," . $did;
		// }else{
		// 	return $did; 
		// }

		// need explode(',',getAllParents(request('id')))
